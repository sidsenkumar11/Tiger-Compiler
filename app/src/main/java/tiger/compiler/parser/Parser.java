...
public class Parser {
    ...
    public void parse() throws ParseException {
        ...
        Symbol stackSymbol;
        do {
            nextToken = tokenList.get(currentTokenNum);
            ...
            if (stackSymbol.isTerminal()) {
                if (!stackSymbol.isEpsilon()) {
                    if (stackSymbol.getValue().equals(nextToken.getValue())) {
                        ...
                        if (backtrackStack.size() > 0) {
                            // So this rule didn't work. Try backtracking and using a different
                            // rule.
                            ...
                            throw new ParseException("Expected " + stackSymbol.getValue()
                                    + " but found " + nextToken.getValue());
                        }
                    }
                }
                ...
                if (stackSymbol.getValue().equalsIgnoreCase("intlit")
                        || stackSymbol.getValue().equalsIgnoreCase("floatlit")) {
                    currentNode.setSymbol(nextToken);
                }

                if (stack.size() != 0 && !(stackSymbol.isDollarToken() && stack.size() == 1)) {
                    while (currentNode != null && (currentNode.getDerivation().size() == 1
                            || currentNode.getDerivation().size() - 1 == currentNode
                                    .getCurrentDeriv()
                            || currentNode.getDerivation().size() == 0)) {
                ...
                if (nextStackSymbols.size() > 1) {
                    for (int i = 1; i < nextStackSymbols.size(); i++) {
                        ...
                        // Pop off those values we just pushed. We don't actually push here; we're
                        // just doing it to save the potential stack state.
                        for (int j = entryInEntry.size() - 1; j >= 0; j--) {
                            stack.pop();
                        }
                    }
                ...
                if (backtrackStack.size() > 0) {
                    ...
                    throw new ParseException("No rule to parse " + stackSymbol.getValue()
                            + " on input " + nextToken.getValue());
                }
            }
            ...
            if (currentToken.charAt(0) == '('
                ...
                if (currentToken.contains("(stmts)")) {
                    ...
                    if (currentToken.indexOf(')') != currentToken.length() - 1) {
                        ...
                        desired = desired.substring(0, indexOfFirstRightParen)
                                + desired.substring(indexOfFirstRightParen + 1);
                        tokens[i] = desired;
                    } else {
    ...
    private void fixTree(ASTNode node) {
        if (!node.getSymbol().isSpecial()) {
            ...
            while (curNode.getDerivation().size() > 1) {
                ...
                curNode.getParent().getDerivation().add(1, curNode.getDerivation().get(0)); // Move
                                                                                            // comma
                                                                                            // up a
                                                                                            // level
            ...
            if (node.getDerivation().size() == 1) {
                ...
                node = node.getParent();
                node.getDerivation().set(index,
                        node.getDerivation().get(index).getDerivation().get(0));
            } else {
                ASTNode curNode = node;
                ...
                if (curNode.getDerivation().size() == 2) {
                    ...
                    while (curNode.getDerivation().size() > 1) {
                        LinkedList<ASTNode> frontPortionOfDerivation = new LinkedList<>(curNode
                                .getDerivation().subList(0, curNode.getDerivation().size() - 2));
                        while (curNode.getDerivation().size() > 2) {
                            curNode.getDerivation().removeFirst();
                        }
                        curNode.getDerivation().addFirst(new ASTNode(new Symbol(false, "numexpr"),
                                frontPortionOfDerivation, 0, 0));
    ...
    public void printAST() {
        this.printAST(root, 0);
        System.out.println("------------------------------------------------------------");
    }

    private void printAST(ASTNode node, int indentation) {
        ...
        System.out.println();
        for (int i = 0; i < node.getDerivation().size(); i++) {
            this.printAST(node.getDerivation().get(i), indentation + 1);
        }
    }
}
